...
do
    ...
    echo "  artifacts:"  >> $STAGE_FILE_PATH
    echo "    name: \"$CI_PROJECT_NAME-$CI_JOB_NAME-$CI_JOB_STARTED_AT\""  >> $STAGE_FILE_PATH
    echo "    paths:"  >> $STAGE_FILE_PATH
    echo "      - $CI_PROJECT_DIR/serverless-ci-output-${TARGET_ENV_NAME}-${TARGET_ENV_PROVIDER_NAME}.properties" >> $STAGE_FILE_PATH
    echo ""  >> $STAGE_FILE_PATH

    # Sam Package Job
    echo "sam-package-${TARGET_ENV_NAME}-${TARGET_ENV_PROVIDER_NAME}:" >> $STAGE_FILE_PATH
    echo "  extends: .abstract-sam-package" >> $STAGE_FILE_PATH
    echo "  rules:" >> $STAGE_FILE_PATH
    echo "    - if: \"\$CI_COMMIT_TITLE =~ /generate CICD stages/\"" >> $STAGE_FILE_PATH
    echo "      when: never" >> $STAGE_FILE_PATH
    echo "    - if: \"\$CI_COMMIT_BRANCH == 'main'\"" >> $STAGE_FILE_PATH
    if [[ "$GLENT_CI_ENVIRONMENT_MANUAL_APPROVAL"  == "true" ]]; then
        echo "      when: manual" >> $STAGE_FILE_PATH
    fi
    ...
    # Create SAM / CloudFormation stack parameters based on environment provider

    SIMPLE_STACK_FILE_NAME="$TARGET_ENV_NAME-$TARGET_ENV_PROVIDER_NAME.json"
    STACK_FILE_PATH="$scriptDir/stackparams/$SIMPLE_STACK_FILE_NAME"
    
    if [[ ! -f "$STACK_FILE_PATH" ]]; then
        cp "$scriptDir/stackparams/template.json" "$STACK_FILE_PATH"

        sed -i "s|AppNameValue|$APP_KEBAB_NAME|g" $STACK_FILE_PATH
        sed -i "s|ComponentIdValue|$APP_SHORT_NAME|g" $STACK_FILE_PATH
        sed -i "s|StageValue|$TARGET_ENV_NAME|g" $STACK_FILE_PATH
        sed -i "s|DomainValue|na|g" $STACK_FILE_PATH
        sed -i "s|VpcIdValue|/${PREFIX}/${TARGET_ENV_PROVIDER_NAME}/vpc|g" $STACK_FILE_PATH
        sed -i "s|VpcSubnetIdsValue|/${PREFIX}/${TARGET_ENV_PROVIDER_NAME}/vpc/private-subnets|g" $STACK_FILE_PATH
        sed -i "s|ResourcePolicySsmValue|/${PREFIX}/${TARGET_ENV_PROVIDER_NAME}/${APP_SHORT_NAME}/resource-binding-policy|g" $STACK_FILE_PATH

        echo "Created stack params file for provider $TARGET_ENV_PROVIDER_NAME"
