  ...
  constructor(parent: Construct, name: string, props: RoleConstructProps) {
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:cloudformation:*:${props.kozmoEnv.awsAccount}:stack/${props.kozmoEnv.prefix}-api-*`],
      })
    );
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        actions: ["states:*"],
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:states:*:${props.kozmoEnv.awsAccount}:*`],
      })
    );
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:dynamodb:*:${props.kozmoEnv.awsAccount}:*`],
      })
    );
    ...
    const roleParam = new ssm.StringParameter(this, `${envIdentifier}-role-param`, {
      allowedPattern: ".*",
      description: `The Role for KOZMO Solution: ${props.kozmoEnv.envName} Environment`,
    ...
    const roleArnParam = new ssm.StringParameter(this, `${envIdentifier}-role-arn-param`, {
      allowedPattern: ".*",
      description: `The Role Arn for KOZMO Solution: ${props.kozmoEnv.envName} Environment`,
