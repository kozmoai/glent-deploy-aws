...
const defaultProps: Partial<ServerlessAPIOperationsConstructProps> = {};

export class ServerlessAPIOperationsConstruct extends Construct {
  public IAMRole: iam.Role;
  public operationsRoleParam: ssm.StringParameter;
  public operationsRoleArnParam: ssm.StringParameter;

  constructor(parent: Construct, name: string, props: ServerlessAPIOperationsConstructProps) {
    super(parent, name);
    ...
    for (const vpc of props.vpcCollection) {
      this.IAMRole.addToPolicy(
        new iam.PolicyStatement({
          actions: ["ec2:*"],
          effect: iam.Effect.ALLOW,
          resources: [`arn:aws:ec2:*:${props.glentEnv.awsAccount}:vpc/${vpc.vpcId}`],
        })
      );
    }
