...
import { DynamoDBConstruct, NetworkConstruct, GLENTEnvironmentParams } from "@aws/aws-app-development-common-constructs";
...
export interface GLENTGenAIEnvStackProps extends cdk.StackProps {
  uniqueEnvIdentifier: string;
}

export class GLENTGenAIEnvStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: GLENTGenAIEnvStackProps) {
    ...
    const provisioningRoleConstruct = new GenAIProvisioningConstruct(
      this,
      `${glentEnvParams.prefix}-${envIdentifier}-provisioning-role`,
      {
        glentEnv: glentEnvParams,
        KMSkey: key,
    ...
    const operationsRoleConstruct = new GenAIOperationsConstruct(
      this,
      `${glentEnvParams.prefix}-${envIdentifier}-operations-role`,
      {
        glentEnv: glentEnvParams,
        KMSkey: key,
        vpcCollection: [network.vpc],
