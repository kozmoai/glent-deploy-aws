...
import { KOZMOServerlessEnvStack } from "./kozmo-serverless-environment-stack";
...
async function main() {
  ...
  // scope: Construct, id: string, props: KOZMOServerlessEnvStackProps
  new KOZMOServerlessEnvStack(app, `SERVERLESS-ENV-${getPrefix()}-${getEnvironmentName()}-Stack`, {
    // stackName: `kozmo-serverless-api-environment`,  // Do not use stack name to get a generated stack name so multiple stacks can be created
    description: `${envID} Serverless Environment for KOZMO(AWS App Development)`,
