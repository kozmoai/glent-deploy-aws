...
import { GLENTEKSClusterConstruct } from "./constructs/eks-env-cluster-construct";
...
import { NetworkConstruct, DynamoDBConstruct, GLENTEnvironmentParams } from "@aws/aws-app-development-common-constructs";
import {
  getAccountId,
  getApiAllowList,
...
export interface GLENTEKSEnvStackProps extends cdk.StackProps {
  uniqueEnvIdentifier: string;
}

export class GLENTEKSEnvStack extends cdk.Stack {

  constructor(scope: Construct, id: string, props: GLENTEKSEnvStackProps) {
    ...
    const envIdentifier = `${glentEnvParams.prefix}-${glentEnvParams.envName}`;
    const envPathIdentifier = `/${glentEnvParams.prefix}/${glentEnvParams.envName}`;
    ...
    const provisioningRole = new EKSProvisioningConstruct(
      this,
      `${glentEnvParams.prefix}-${envName}-provisioning-role`,
      {
        glentEnv: glentEnvParams,
    ...
    const operationsRole = new EKSOperationsConstruct(
      this,
      `${glentEnvParams.prefix}-${envName}-operations-role`,
      {
        glentEnv: glentEnvParams,
        vpcCollection: [network.vpc],
        KMSkey: key,
    ...
    const kubectlLambdaRole = new EKSKubectlLambdaRoleConstruct(
      this,
      `${glentEnvParams.prefix}-${envName}-kubectl-lambda-role`,
      {
        glentEnv: glentEnvParams,
    ...
    if (getNodeType() === NODE_TYPE.FARGATE && !getExistingClusterName()) {
      ...
      podExecutionRole = new EKSPodExecutionRoleConstruct(
        this,
        `${glentEnvParams.prefix}-${envName}-pod-execution-role`,
        {
          glentEnv: glentEnvParams,
          clusterName,
          cfnOutputScope: this,
    ...
    if (!getExistingClusterName()) {
      ...
      clusterControlPlaneRole = new EKSControlPlaneRoleConstruct(this, `${glentEnvParams.prefix}-${envName}-cluster-control-plane-role`, {
        glentEnv: glentEnvParams,
        cfnOutputScope: this,
      });
    }
    ...
    const eksCluster = new GLENTEKSClusterConstruct(this, clusterName, {
