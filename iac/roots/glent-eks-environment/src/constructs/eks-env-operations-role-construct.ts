...
export class EKSOperationsConstruct extends Construct {
  ...
  constructor(parent: Construct, name: string, props: EKSOperationsConstructProps) {
    ...
    const truncatedName = `EKS-ENV-${props.glentEnv.prefix}-${props.glentEnv.envName}`.substring(0, 34);
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        actions: [
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:lambda:${props.glentEnv.awsRegion}:${props.glentEnv.awsAccount}:function:${truncatedName}*`],
      })
    );
