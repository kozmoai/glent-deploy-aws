...
import {
  getCreateK8sGlentResources,
  getExistingClusterName,
  getExistingKubectlOnEventLambdaArn,
  getNodeType,
  NODE_TYPE,
} from "../eks-input";
import { GLENTEKSManagedNodeClusterConstruct } from "./eks-env-cluster-managed-node";
import { GLENTEKSFargateClusterConstruct } from "./eks-env-cluster-fargate";
...
export interface GLENTEKSClusterConstructProps extends cdk.StackProps {
...
export class GLENTEKSClusterConstruct extends Construct {
  public readonly clusterParameter: ssm.StringParameter;
  public readonly cluster: eks.Cluster;

  constructor(parent: Construct, name: string, props: GLENTEKSClusterConstructProps) {
    super(parent, name);
    ...
    if (isCreateNewCluster) {
      ...
      if (getNodeType() === NODE_TYPE.MANAGED) {
        clusterConstruct = new GLENTEKSManagedNodeClusterConstruct(this, `${envIdentifier}-app-runtime`, {
          cfnOutputScope: props.cfnOutputScope,
          glentEnv: props.glentEnv,
        ...
        clusterConstruct = new GLENTEKSFargateClusterConstruct(this, `${envIdentifier}-app-runtime`, {
    ...
    if (getCreateK8sGlentResources()) {
      ...
      const clusterAdminClusterRoleBinding = cluster.addManifest(`cluster-admin-role-binding`, {
        ...
        metadata: { name: 'glent-cluster-admin' },
        subjects: [
          {
            kind: 'User',
      ...
      const clusterNamespaceViewerRole = cluster.addManifest(`cluster-ns-viewer-role`, {
        apiVersion: 'rbac.authorization.k8s.io/v1',
        kind: 'ClusterRole',
        metadata: { name: 'glent-namespace-viewer' },
        rules: [
          {
            apiGroups: [''],
    ...
    } else if (!getExistingKubectlOnEventLambdaArn() && !getCreateK8sGlentResources()) {

      // GLENT requires the KubeCtl Provider, which is typically a lambda function
    ...
    const clusterParam = new ssm.StringParameter(this, `${name}-eks-cluster-param`, {
      allowedPattern: ".*",
      description: `The EKS Cluster for GLENT Solution: ${props.glentEnv.envName} Environment`,
