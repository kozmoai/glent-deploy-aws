...
export interface BasicOperationsConstructProps extends cdk.StackProps {
...
const defaultProps: Partial<BasicOperationsConstructProps> = {};

export class BasicOperationsConstruct extends Construct {
  public IAMRole: iam.Role;
  public operationsRoleParam: ssm.StringParameter;
  public operationsRoleArnParam: ssm.StringParameter;

  constructor(parent: Construct, name: string, props: BasicOperationsConstructProps) {
    super(parent, name);
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:dynamodb:*:${props.glentEnv.awsAccount}:table/*`],
      })
    );
    ...
    // allow deploying a serverless application stack (such as a SAM template)
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        actions: [
          "cloudformation:CreateChangeSet",
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:cloudformation:*:${props.glentEnv.awsAccount}:stack/*`],
        conditions: {
          "StringEquals": {
            "aws:ResourceAccount": props.glentEnv.awsAccount
          }
    ...
    this.IAMRole.addManagedPolicy(iam.ManagedPolicy.fromAwsManagedPolicyName("AmazonS3ReadOnlyAccess"));

    // now save the VPC in SSM Param
    const roleParam = new ssm.StringParameter(this, `${envIdentifier}-role-param`, {
      allowedPattern: ".*",
      description: `The Operations Role for GLENT Solution: ${props.glentEnv.envName} Environment`,
    ...
    const roleArnParam = new ssm.StringParameter(this, `${envIdentifier}-role-arn-param`, {
      allowedPattern: ".*",
      description: `The Operations Role Arn for GLENT Solution: ${props.glentEnv.envName} Environment`,
