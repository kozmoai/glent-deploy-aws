...
import { GLENTEnvironmentParams, DynamoDBConstruct, } from '@aws/aws-app-development-common-constructs'
...
export interface GLENTBasicEnvStackProps extends cdk.StackProps {
  uniqueEnvIdentifier: string;
}

export class GLENTBasicEnvStack extends cdk.Stack {

  constructor(scope: Construct, id: string, props: GLENTBasicEnvStackProps) {
    super(scope, id, props);

    const prefix = process.env.PREFIX as string || "glent";
    ...
    const provisioningRoleConstruct = new BasicProvisioningConstruct(this, `${glentEnvParams.prefix}-${envIdentifier}-provisioning-role`, {
      glentEnv: glentEnvParams,
      KMSkey: key,
    ...
    const operationsRoleConstruct = new BasicOperationsConstruct(this, `${glentEnvParams.prefix}-${envIdentifier}-operations-role`, {
      glentEnv: glentEnvParams,
      KMSkey: key,
