...
import { NetworkConstruct, KOZMOEnvironmentParams, EcsClusterConstruct, DynamoDBConstruct, } from '@aws/aws-app-development-common-constructs'
...
export class KOZMOECSEnvStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: KOZMOECSEnvStackProps) {
    ...
    const provisioningRoleConstruct = new ECSProvisioningConstruct(this, `${kozmoEnvParams.prefix}-${envName}-provisioning-role`, {
      kozmoEnv: kozmoEnvParams,
      KMSkey: key,
    ...
    const operationsRoleConstruct = new ECSOperationsConstruct(this, `${kozmoEnvParams.prefix}-${envName}-operations-role`, {
      kozmoEnv: kozmoEnvParams,
      KMSkey: key,
      vpcCollection: [network.vpc],
      ecsCollection: [ecsAppCluster.cluster],
