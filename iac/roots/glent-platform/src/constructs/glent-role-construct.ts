...
import { GLENTEnvironmentParams, NetworkConstruct } from "@aws/aws-app-development-common-constructs";
...
export interface GLENTRootRoleConstructProps extends cdk.StackProps {
...
const defaultProps: Partial<GLENTRootRoleConstructProps> = {};

export class GLENTRootRoleConstruct extends Construct {
  public IAMRole: iam.Role;
  private props: GLENTRootRoleConstructProps;

  constructor(parent: Construct, name: string, props: GLENTRootRoleConstructProps) {
    super(parent, name);
    ...
    this.IAMRole = new iam.Role(this, name, {
      ...
      inlinePolicies: {
        [`${name}-role-assumption`]: this.createRoleAssumptionPolicyDocument(this.props.glentEnv.awsAccount),
      },
    ...
    const roleParam = new ssm.StringParameter(this, `${this.props.glentEnv.prefix}-platform-role`, {
      allowedPattern: ".*",
      description: `The GLENT Backstage Platform Role Arn`,
      parameterName: `/${this.props.glentEnv.prefix}/platform-role`,
  ...
  private createSecretsManagerStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      resources: [`arn:aws:secretsmanager:*:${this.props.glentEnv.awsAccount}:secret:*`],
    });
  }
  ...
  private createS3BucketStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      conditions: {
        StringEquals: {
          "aws:ResourceAccount": this.props.glentEnv.awsAccount,
        },
      },
    });
  }
  ...
  private createResourceGroupsStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:resource-groups:*:${this.props.glentEnv.awsAccount}:group/*`],
    });
  }
  ...
  private createCloudformationStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:cloudformation:*:${this.props.glentEnv.awsAccount}:stack/${this.props.glentEnv.prefix}-api-*`],
    });
  }
  ...
  private createEc2Statement() {
    return new iam.PolicyStatement({
      actions: ["ec2:*"],
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:ec2:*:${this.props.glentEnv.awsAccount}:vpc/${this.props.network.vpc.vpcId}`],
    });
  }
  ...
  private createDyamodbStatement() {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:dynamodb:*:${this.props.glentEnv.awsAccount}:table/${this.props.securityTableName}`],
    });
  }
  ...
  private createRoleAssumptionPolicyDocument(account: string): cdk.aws_iam.PolicyDocument {
    ...
    const glentRoleAssumptionPolicy = new iam.PolicyStatement({
      sid: "AllowGLENTRoleAssumptionInAccount",
      actions: ["sts:AssumeRole"],
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:iam::${account}:role/glent*`],
    });
