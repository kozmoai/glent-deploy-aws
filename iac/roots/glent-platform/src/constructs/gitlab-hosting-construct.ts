...
export class GitlabHostingConstruct extends Construct {
  ...
  constructor(parent: Construct, name: string, props: GitlabHostingConstructProps) {
    ...
    new ssm.StringParameter(this, `${props.glentEnv.prefix}-gitlab-security-group`, {
      allowedPattern: ".*",
      description: `The Gitlab LB Security Group`,
      parameterName: `/${props.glentEnv.prefix}/gitlab-security-group`,
      stringValue: albSecurityGroup.securityGroupId,
    });

    const glRunnerSgName = `${props.glentEnv.prefix}-alb-gitlab-runner-sg`;
    ...
    const gitlabHost = new ec2.Instance(this, "GitlabHost", {
      instanceName: `${props.glentEnv.prefix}-GitlabHost`,
    ...
    // create ALB
    const gitlabAlb = new elb.ApplicationLoadBalancer(this, `${props.glentEnv.prefix}-gitlab-alb`, {
      vpc: props.network.vpc,
      vpcSubnets: { subnetType: ec2.SubnetType.PUBLIC },
      loadBalancerName: `${props.glentEnv.prefix}-gitlab-alb`,
    ...
    const aRecord = new route53.ARecord(this, `${props.glentEnv.prefix}-r53-aliasrecord`, {
    ...
    const httpsListener = gitlabAlb.addListener(`${props.glentEnv.prefix}-alb-httpsListener`, {
      protocol: elb.ApplicationProtocol.HTTPS,
      open: false,
      certificates: [props.hostedZone?.certificate!],
    });

    httpsListener.addTargets(`${props.glentEnv.prefix}-target-grp2`, {
      protocol: elb.ApplicationProtocol.HTTP,
      targetGroupName: `${props.glentEnv.prefix}-git-target-grp2`,
    ...
    lbDNSParam = new ssm.StringParameter(this, `${props.glentEnv.prefix}-gitlab-hostname`, {
      allowedPattern: ".*",
      description: `The Gitlab Hostname for GLENT Platform`,
      parameterName: `/${props.glentEnv.prefix}/gitlab-hostname`,
      stringValue: aRecord.domainName,
    });

    lbUrlParam = new ssm.StringParameter(this, `${props.glentEnv.prefix}-gitlab-url`, {
      allowedPattern: ".*",
      description: `The Gitlab URL for GLENT Platform`,
      parameterName: `/${props.glentEnv.prefix}/gitlab-url`,
