...
export class GitlabRunnerConstruct extends Construct {
  constructor(parent: Construct, name: string, props: GitlabRunnerConstructProps) {
    ...
    const glRunnerEc2SgName = `${props.glentEnv.prefix}-gitlab-runner-sg`;
    ...
    const gitlabIamRolePolicy = new iam.PolicyDocument({
      statements: [
        ...
        //    "arn:aws:iam::123456789012:role/glent-environment-provisioning-role"
        new iam.PolicyStatement({
          actions: [
            "sts:AssumeRole",
          ],
    ...
    const roleParam = new ssm.StringParameter(this, `${props.glentEnv.prefix}-pipeline-role`, {
      allowedPattern: ".*",
      description: `The GLENT Platform Pipeline Role Arn`,
      parameterName: `/${props.glentEnv.prefix}/pipeline-role`,
      stringValue: gitlabEc2Role.roleArn,
    });

    new cdk.CfnOutput(this, `The GLENT Platform Pipeline Role Arn Parameter`, {
