  ...
  constructor(parent: Construct, name: string, props: RoleConstructProps) {
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:cloudformation:*:${props.glentEnv.awsAccount}:stack/${props.glentEnv.prefix}-api-*`],
      })
    );
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        actions: ["states:*"],
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:states:*:${props.glentEnv.awsAccount}:*`],
      })
    );
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:dynamodb:*:${props.glentEnv.awsAccount}:*`],
      })
    );
    ...
    const roleParam = new ssm.StringParameter(this, `${envIdentifier}-role-param`, {
      allowedPattern: ".*",
      description: `The Role for GLENT Solution: ${props.glentEnv.envName} Environment`,
    ...
    const roleArnParam = new ssm.StringParameter(this, `${envIdentifier}-role-arn-param`, {
      allowedPattern: ".*",
      description: `The Role Arn for GLENT Solution: ${props.glentEnv.envName} Environment`,
