...
import {
  getCreateK8sKozmoResources,
  getExistingClusterName,
  getExistingKubectlOnEventLambdaArn,
  getNodeType,
  NODE_TYPE,
} from "../eks-input";
import { KOZMOEKSManagedNodeClusterConstruct } from "./eks-env-cluster-managed-node";
import { KOZMOEKSFargateClusterConstruct } from "./eks-env-cluster-fargate";
...
export interface KOZMOEKSClusterConstructProps extends cdk.StackProps {
...
export class KOZMOEKSClusterConstruct extends Construct {
  public readonly clusterParameter: ssm.StringParameter;
  public readonly cluster: eks.Cluster;

  constructor(parent: Construct, name: string, props: KOZMOEKSClusterConstructProps) {
    super(parent, name);
    ...
    if (isCreateNewCluster) {
      ...
      if (getNodeType() === NODE_TYPE.MANAGED) {
        clusterConstruct = new KOZMOEKSManagedNodeClusterConstruct(this, `${envIdentifier}-app-runtime`, {
          cfnOutputScope: props.cfnOutputScope,
          kozmoEnv: props.kozmoEnv,
        ...
        clusterConstruct = new KOZMOEKSFargateClusterConstruct(this, `${envIdentifier}-app-runtime`, {
    ...
    if (getCreateK8sKozmoResources()) {
      ...
      const clusterAdminClusterRoleBinding = cluster.addManifest(`cluster-admin-role-binding`, {
        ...
        metadata: { name: 'kozmo-cluster-admin' },
        subjects: [
          {
            kind: 'User',
      ...
      const clusterNamespaceViewerRole = cluster.addManifest(`cluster-ns-viewer-role`, {
        apiVersion: 'rbac.authorization.k8s.io/v1',
        kind: 'ClusterRole',
        metadata: { name: 'kozmo-namespace-viewer' },
        rules: [
          {
            apiGroups: [''],
    ...
    } else if (!getExistingKubectlOnEventLambdaArn() && !getCreateK8sKozmoResources()) {

      // KOZMO requires the KubeCtl Provider, which is typically a lambda function
    ...
    const clusterParam = new ssm.StringParameter(this, `${name}-eks-cluster-param`, {
      allowedPattern: ".*",
      description: `The EKS Cluster for KOZMO Solution: ${props.kozmoEnv.envName} Environment`,
