...
import { KOZMOEKSClusterConstruct } from "./constructs/eks-env-cluster-construct";
...
import { NetworkConstruct, DynamoDBConstruct, KOZMOEnvironmentParams } from "@aws/aws-app-development-common-constructs";
import {
  getAccountId,
  getApiAllowList,
...
export interface KOZMOEKSEnvStackProps extends cdk.StackProps {
  uniqueEnvIdentifier: string;
}

export class KOZMOEKSEnvStack extends cdk.Stack {

  constructor(scope: Construct, id: string, props: KOZMOEKSEnvStackProps) {
    ...
    const envIdentifier = `${kozmoEnvParams.prefix}-${kozmoEnvParams.envName}`;
    const envPathIdentifier = `/${kozmoEnvParams.prefix}/${kozmoEnvParams.envName}`;
    ...
    const provisioningRole = new EKSProvisioningConstruct(
      this,
      `${kozmoEnvParams.prefix}-${envName}-provisioning-role`,
      {
        kozmoEnv: kozmoEnvParams,
    ...
    const operationsRole = new EKSOperationsConstruct(
      this,
      `${kozmoEnvParams.prefix}-${envName}-operations-role`,
      {
        kozmoEnv: kozmoEnvParams,
        vpcCollection: [network.vpc],
        KMSkey: key,
    ...
    const kubectlLambdaRole = new EKSKubectlLambdaRoleConstruct(
      this,
      `${kozmoEnvParams.prefix}-${envName}-kubectl-lambda-role`,
      {
        kozmoEnv: kozmoEnvParams,
    ...
    if (getNodeType() === NODE_TYPE.FARGATE && !getExistingClusterName()) {
      ...
      podExecutionRole = new EKSPodExecutionRoleConstruct(
        this,
        `${kozmoEnvParams.prefix}-${envName}-pod-execution-role`,
        {
          kozmoEnv: kozmoEnvParams,
          clusterName,
          cfnOutputScope: this,
    ...
    if (!getExistingClusterName()) {
      ...
      clusterControlPlaneRole = new EKSControlPlaneRoleConstruct(this, `${kozmoEnvParams.prefix}-${envName}-cluster-control-plane-role`, {
        kozmoEnv: kozmoEnvParams,
        cfnOutputScope: this,
      });
    }
    ...
    const eksCluster = new KOZMOEKSClusterConstruct(this, clusterName, {
