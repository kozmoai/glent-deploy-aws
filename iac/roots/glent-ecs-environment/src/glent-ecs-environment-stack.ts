...
import { NetworkConstruct, GLENTEnvironmentParams, EcsClusterConstruct, DynamoDBConstruct, } from '@aws/aws-app-development-common-constructs'
...
export interface GLENTECSEnvStackProps extends cdk.StackProps {
  uniqueEnvIdentifier: string;
}

export class GLENTECSEnvStack extends cdk.Stack {

  constructor(scope: Construct, id: string, props: GLENTECSEnvStackProps) {
    ...
    const provisioningRoleConstruct = new ECSProvisioningConstruct(this, `${glentEnvParams.prefix}-${envName}-provisioning-role`, {
      glentEnv: glentEnvParams,
      KMSkey: key,
    ...
    const operationsRoleConstruct = new ECSOperationsConstruct(this, `${glentEnvParams.prefix}-${envName}-operations-role`, {
      glentEnv: glentEnvParams,
      KMSkey: key,
      vpcCollection: [network.vpc],
      ecsCollection: [ecsAppCluster.cluster],
