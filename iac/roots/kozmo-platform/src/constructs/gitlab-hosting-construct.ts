...
import { KOZMOEnvironmentParams, HostedZoneConstruct, NetworkConstruct } from "@aws/aws-app-development-common-constructs";
...
export class GitlabHostingConstruct extends Construct {
  ...
  constructor(parent: Construct, name: string, props: GitlabHostingConstructProps) {
    ...
    new ssm.StringParameter(this, `${props.kozmoEnv.prefix}-gitlab-security-group`, {
      allowedPattern: ".*",
      description: `The Gitlab LB Security Group`,
      parameterName: `/${props.kozmoEnv.prefix}/gitlab-security-group`,
      stringValue: albSecurityGroup.securityGroupId,
    });

    const glRunnerSgName = `${props.kozmoEnv.prefix}-alb-gitlab-runner-sg`;
    ...
    const gitlabHost = new ec2.Instance(this, "GitlabHost", {
      instanceName: `${props.kozmoEnv.prefix}-GitlabHost`,
    ...
    // create ALB
    const gitlabAlb = new elb.ApplicationLoadBalancer(this, `${props.kozmoEnv.prefix}-gitlab-alb`, {
      vpc: props.network.vpc,
      vpcSubnets: { subnetType: ec2.SubnetType.PUBLIC },
      loadBalancerName: `${props.kozmoEnv.prefix}-gitlab-alb`,
    ...
    const aRecord = new route53.ARecord(this, `${props.kozmoEnv.prefix}-r53-aliasrecord`, {
    ...
    const httpsListener = gitlabAlb.addListener(`${props.kozmoEnv.prefix}-alb-httpsListener`, {
      protocol: elb.ApplicationProtocol.HTTPS,
      open: false,
      certificates: [props.hostedZone?.certificate!],
    });

    httpsListener.addTargets(`${props.kozmoEnv.prefix}-target-grp2`, {
      protocol: elb.ApplicationProtocol.HTTP,
      targetGroupName: `${props.kozmoEnv.prefix}-git-target-grp2`,
    ...
    lbDNSParam = new ssm.StringParameter(this, `${props.kozmoEnv.prefix}-gitlab-hostname`, {
      allowedPattern: ".*",
      description: `The Gitlab Hostname for KOZMO Platform`,
      parameterName: `/${props.kozmoEnv.prefix}/gitlab-hostname`,
      stringValue: aRecord.domainName,
    });

    lbUrlParam = new ssm.StringParameter(this, `${props.kozmoEnv.prefix}-gitlab-url`, {
      allowedPattern: ".*",
      description: `The Gitlab URL for KOZMO Platform`,
      parameterName: `/${props.kozmoEnv.prefix}/gitlab-url`,
