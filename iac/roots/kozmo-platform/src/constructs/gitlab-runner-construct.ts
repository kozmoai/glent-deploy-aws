...
export class GitlabRunnerConstruct extends Construct {
  constructor(parent: Construct, name: string, props: GitlabRunnerConstructProps) {
    ...
    const glRunnerEc2SgName = `${props.kozmoEnv.prefix}-gitlab-runner-sg`;
    ...
    const gitlabIamRolePolicy = new iam.PolicyDocument({
      statements: [
        ...
        //    "arn:aws:iam::123456789012:role/kozmo-environment-provisioning-role"
        new iam.PolicyStatement({
          actions: [
            "sts:AssumeRole",
          ],
    ...
    const roleParam = new ssm.StringParameter(this, `${props.kozmoEnv.prefix}-pipeline-role`, {
      allowedPattern: ".*",
      description: `The KOZMO Platform Pipeline Role Arn`,
      parameterName: `/${props.kozmoEnv.prefix}/pipeline-role`,
      stringValue: gitlabEc2Role.roleArn,
    });

    new cdk.CfnOutput(this, `The KOZMO Platform Pipeline Role Arn Parameter`, {
