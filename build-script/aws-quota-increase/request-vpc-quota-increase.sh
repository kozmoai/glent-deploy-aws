...
# the maximum allowed VPCs for an AWS account in
# a specific region.
# Script Arguments
#   arg1: the maximum number of VPCs for the AWS account in
#         a specific region.
#   arg2: the AWS region.
...
if [[ -z "$newValue" ]]; then
    echo -e "${RED} ERROR: The desired number for the maximum allowed VPCs must be passed as the first argument to this script.${NC}" >&2
    exit 1
fi
...
if [[ "$mode" == "interactive" ]]; then
    read -p "You are about to request a VPC quota increase to $newValue for account $cliAccountId in region $region. Do you want to proceed (y/n)? [n] " proceed
...
echo ""
echo "Requesting VPC quota increase to $newValue for account $cliAccountId in region $region..."
