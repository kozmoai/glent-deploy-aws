...
if [[ -z $AWS_ACCOUNT_ID ]]; then
  ...
  aws_account_number AWS_ACCOUNT_ID "Enter the AWS account number where the GLENT platform was deployed:" "$defaultAwsAccountNum"
fi
...
if [[ -z $AWS_DEFAULT_REGION ]]; then
  ...
  aws_region AWS_DEFAULT_REGION "Enter the AWS region where the GLENT platform was deployed:" "$defaultRegion"
fi
